...
3. **No permission at all** — Cooking/Crafting are server-wide (open to all when `base.enabled` is true) and Skills gates via a single runtime node.
...
- **LeetCrafting** registers **no** feature permission — Cooking and Crafting declare no `base.permission` key, and `registerPermission()` skips a node when the config lacks the key. They are server-wide.
...
| Cooking | LeetCrafting | — (none) | server-wide |
