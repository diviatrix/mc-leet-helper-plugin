 ...
 */
public class XpFeature extends AbstractFeature implements MessagingFeature {
    ...
    public XpFeature(CoreApi core, JavaPlugin owner) {
        super(core, owner);
    }
    ...
    protected void loadFeatureConfig(YamlConfiguration cfg) {
        ...
        if (persistentPlaced && !core.storageManager().persistentAvailable()) {
            owner.getLogger().warning("feature.placed-tracking is 'persistent' but SQLite is unavailable; "
        ...
        if (mobs != null) {
            for (String name : mobs.getKeys(false)) {
                try {
                    ...
                    owner.getLogger().warning("Invalid entity in xp killing mobs: " + name);
                }
            }
        }
    }
    ...
    private void prunePlaced() {
        ...
        if (persistentPlaced) {
            core.storageManager().prunePersistent(featureId(), "placed", cutoff);
        } else {
    ...
    private void markPlaced(Block block) {
        if (persistentPlaced) {
            core.storageManager().setPersistent(featureId(), "placed", blockUuid(block),
    ...
    private boolean isPlacedAndConsume(Block block) {
        if (persistentPlaced) {
            ...
            String stored = core.storageManager().getPersistent(featureId(), "placed", id);
            if (stored == null) return false;
            core.storageManager().deletePersistent(featureId(), "placed", id);
            return true;
        }
